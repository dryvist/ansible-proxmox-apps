...
#
# This is a drop-in link in the SAME chain as `sops exec-env` / `doppler
# run --` — it sits between them and whatever it wraps, and does exactly the
# same job: get secrets into the process environment before the wrapped
# command starts. Nothing downstream (an Ansible role's `lookup('env', 'X')`,
# a Terraform `TF_VAR_x` default, a CI step) can tell OpenBao was involved at
# all — swap this script for a different one and the config is unchanged.
#
# Usage:
...
# (e.g. "media" -> MEDIA, "local-llm" -> LOCAL_LLM) — matching the naming the
# roles/openbao_secrets Ansible role already uses for the same AppRoles.
#
...
# same posture as roles/openbao_secrets, so a host/domain not yet migrated to
# OpenBao is unaffected.
