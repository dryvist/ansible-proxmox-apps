...
zdb = seed_ticket(
  ...
  articles: [
    { subject: 'Incident', body: "Zammad web 500s; the guest cannot reach its database. Initially read as a password desync from the postgres-apps bring-up (apps#1005 lane), but a converge on 2026-07-17 00:51Z showed otherwise: `zammad run rake db:migrate` fails trying to CREATE the database, connecting to the `postgres` maintenance DB, which pg_hba rejects — 'no pg_hba.conf entry for host <guest>, user zammad, database postgres'. So the zammad database does not exist on postgres.jacobpevans.com, which is where the guest points (POSTGRES_PRIMARY_HOST is unset, so group_vars/all falls back to 'postgres' while the postgres-apps migration is in flight). NETWORK_CIDR_APPS does cover the guest — pg_hba entries are per-database and only the zammad database is granted from that CIDR, so the maintenance-DB connection is correctly refused." },
...
# ============================================================================
# Today's incident (2026-07-17) — Default LAN deleted then restored
# ============================================================================

seed_ticket(
  title: 'Default LAN (UniFi lan_main) deleted then restored — self-inflicted (2026-07-17)',
  group: hi, state: closed, priority: high, customer: admin,
